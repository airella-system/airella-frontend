...
class DetailedChartTabs extends Component {
  constructor(props) {
    super(props);
    this.state = {
      activeTab: 0,
      ...
      options: {
        ...
        tooltips: {
          ...
          callbacks: {
            label: function (tooltipItem, data) {
              return (
                tooltipLabel[tooltipItem.datasetIndex] +
                " " +
                tooltipItem.yLabel +
                "µg/m³"
              );
  ...
  makePmChart() {
    ...
    for (let index of ["pm1", "pm2_5", "pm10"]) {
      let sensor = this.state.data.sensors.filter(
  ...
  render() {
    return <canvas id="pmChart" className="chart" width="100"></canvas>;
  }
}
