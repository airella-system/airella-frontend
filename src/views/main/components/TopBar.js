...
class TopBar extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
	  if (prevState.searchText != this.state.searchText) {
		...
		this.fetchTimeout = setTimeout(()=>{this.fetchSearch();}, 500);
	  }
  }
  ...
  setSearchAreaRef = (node) => {
	...
	getAutoCompleteList = () => {
		if (!this.state.searchData) {
			return <ol className="autoCompleteBox autoCompleteBoxHidden">
			</ol>
		}
		...
		const listItems = limitedFeatures.map((feature, index) =>
			<li key={index} onClick={() => this.handleSuggestionClick(feature)}>
				...
				<br></br>
				{feature.properties.country != null && feature.properties.city != null &&
					<span className="suggestionSubtitle">{`${feature.properties.country}, ${feature.properties.city}`}</span>}
			</li>
		)
		
		return <ol className={`autoCompleteBox ${this.state.searchFocus ? "autoCompleteBoxVisible" : "autoCompleteBoxHidden"}`}>
			{listItems}
		</ol>
	}
